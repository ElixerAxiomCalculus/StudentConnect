...
import {
    User, Lock, Bell, Accessibility, Save, Camera, X as XIcon
} from 'lucide-react';
...
export default function SettingsPage() {
    ...
    const handleAddInterest = (e) => {
        if (e.key === 'Enter' && e.target.value.trim()) {
            setProfile((p) => ({
                ...p,
                interests: [...(p.interests || []), e.target.value.trim()],
            }));
            e.target.value = '';
        }
    };
            ...
            <FrostCard flat className="settings-section">
                ...
                <div className="avatar-upload" style={{ marginBottom: 20 }}>
                    ...
                    <button className="avatar-upload-btn">
                        <Camera size={14} style={{ marginRight: 6 }} /> Change Avatar
                    </button>
                </div>
                    ...
                    <div className="interest-tags">
                        {profile.interests?.map((interest) => (
                            <span key={interest} className="interest-tag">
                                {interest}
                                <button className="interest-tag-remove" onClick={() => handleInterestRemove(interest)} aria-label={`Remove ${interest}`}>
                                    <XIcon size={10} />
                                </button>
                            </span>
                        ))}
